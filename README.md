...
Это необходимо для того, чтобы презентер мог узнать об изменении состояния приложения и обновить представление.
...
Пользователь → View → EventEmitter → Presenter → Model  
Model → EventEmitter → Presenter → View
